...
const ManageDataset: React.FC<ManageDatasetProps> = ({ dataset }) => {
	...
	const [deleteDatasetKind, setDeleteDatasetKind] = React.useState<
		string | null
	>(null)
	...
	const queryUrl = React.useMemo(
		() => `/dataset-kinds?datasetId=${dataset.id}`,
		[dataset.id]
	)
	const query =
		useGetRequest<{ kind: string; current?: number; total: number }[]>(
			queryUrl
		)
	...
	const onDelete = React.useCallback(
		() => deleteQuery.request({ id: dataset.id }),
		[dataset.id, deleteQuery.request]
	)
	...
	return (
		...
		>
			...
			>
				<Label
					text={`Are you sure you want to delete ${dataset.name}? This operation is irreversible!`}
				/>
				<Button
					colorVariant='error'
					label={`Delete ${dataset.name}`}
					onClick={onDelete}
				/>
			</Popup>
			<Popup
			...
			>
				...
				<Button
					colorVariant='error'
					label={`Delete kind ${deleteDatasetKind}`}
					onClick={onDeleteKind}
				/>
			</Popup>
			<Popup
				title='Update Dataset Kinds'
				open={editKindsOpen}
				onClose={() => setEditKindsOpen(false)}
			>
...
export function AdminPage() {
	...
	return (
		<Page>
			...
			{query.state === 'success' && (
				<Label text='Successfully Uploaded' color='success' />
			)}
			{query.state === 'error' && (
				<Label text='Upload Failed. See Console (F12) for details' />
			)}
		</Page>
	)
}
